...
function opposingTeam($array, $firstteam)
{
    $arrayData = $array;
...
function createTeams($finalResultArray)
{
    $Teams = [];
    // this codes used to get the available teams from the finalResultArray ;
    $TheTeamsKeys = array();
    // key will be the teamvsteam2
    // val will be the array containing the goals scored by each team and if the match played
    foreach ($finalResultArray as $key => $val) {
        $contries = array();
        $values = array();
    ...
    foreach ($Teams as $key => $value) {
        ...
        foreach ($finalResultArray as $DataKey => $DataValue) {
            // condition : if score of team is not 0             
            if (isset($DataValue[$key])) {
                $GOALS_SCORED += $DataValue[$key];
                $GOALS_RECEIVED += $DataValue[opposingTeam($DataValue, $key)];
                $DIFF = $GOALS_SCORED - $GOALS_RECEIVED;
                if ($DataValue["Played"] == true) {
                    $GAMES_PLAYED += 1;
                }
                if ($DataValue[$key] > $DataValue[opposingTeam($DataValue, $key)]) {
                    $GAMES_WON += 1;
                } elseif ($DataValue[$key] < $DataValue[opposingTeam($DataValue, $key)]) {
                    $GAME_LOSTS += 1;
                } elseif ($DataValue[$key] == $DataValue[opposingTeam($DataValue, $key)]) {
                    $GAMES_EQUAL += 1;
                }
    ...
    return changeArrayForm($Teams);
};

function changeArrayForm($data)
{
